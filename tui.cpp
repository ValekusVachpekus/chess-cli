...
void drawBoard(ChessFacade &game, int cursorX, int cursorY,
    ...
    for (int sy = 7; sy >= 0; sy--) {
        ...
        for (int sx = 0; sx < 8; sx++) {
            ...
            ChessFacade::PieceView piece =
                game.getPieceAt(Coordinates(bx, by));
            string cell = ".";
            int colorPair = 0;
            if (isValid) {
                if (piece.present) {
                ...
                cell = typeToString(piece.type);
                colorPair = (piece.color == WHITE) ? COLOR_WHITE_PIECE
                                                   : COLOR_BLACK_PIECE;
            }
...
int main() {
    ...
    if (game.isGameOver()) {
        drawBoard(game, cursorX, cursorY, {}, game.getLastMessage(), turn,
                  flipped, flipOnTurn);
        getch();
    }
