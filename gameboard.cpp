...
class IBoard {
  public:
    ...
    virtual void moveFigure(const Coordinates &oldc,
    ...
    void setFigure(Figure *figure) override {
        if (!figure->getCoordinates().canMove()) {
            cerr << "Invalid coordinates for setFigure: "
                 << figure->getCoordinates().getX()
                 << figure->getCoordinates().getY() << endl;
            return;
        }
  ...
  public:
    ...
    vector<Coordinates> getValidMoves() override {
        ...
        if (rightTarget != nullptr &&
            rightTarget->getColor() != this->getColor()) {
  ...
  public:
    static vector<Coordinates> getOrthogonalMoves(Figure *figure) {
        ...
        for (int x1 = x + 1; x1 < 8; x1++) {
            IFigure *target =
                figure->getGameboard()->getFigure(Coordinates(x1, y));
            if (target == nullptr) {
        ...
        for (int y1 = y + 1; y1 < 8; y1++) {
            IFigure *target =
                figure->getGameboard()->getFigure(Coordinates(x, y1));
            if (target == nullptr) {
  ...
  public:
    static vector<Coordinates> getDiagonalMoves(Figure *figure) {
        ...
        for (int x1 = x + 1, y1 = y + 1; x1 < 8 && y1 < 8; x1++, y1++) {
            IFigure *target =
                figure->getGameboard()->getFigure(Coordinates(x1, y1));
            if (target == nullptr) {
...
void Pawn::prevrashenie(Type type) {
    ...
    switch (type) {
    case QUEEN:
        newFigure = new Queen(this->getId(), this->getColor(),
    ...
    case ELEPHANT:
        ...
        break;
    case LADYA:
        newFigure = new Ladya(this->getId(), this->getColor(),
                              this->getGameboard(), pos);
        break;
    case HORSE:
        newFigure = new Horse(this->getId(), this->getColor(),
                              this->getGameboard(), pos);
        break;
    default:
        newFigure = new Queen(this->getId(), this->getColor(),
...
class King : public Figure {
  private:
    ...
    bool isSquareUnderAttack(const Coordinates &square) const {
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                IFigure *fig =
                    this->getGameboard()->getFigure(Coordinates(x, y));
  ...
  public:
    ...
    bool isInCheck() const {
        return isSquareUnderAttack(this->getCoordinates());
    }
    ...
    vector<Coordinates> getValidMoves() override {
        ...
        if (!hadMoved && x == 4 && (y == 0 || y == 7)) {
            ...
            Ladya *kingRook = (kingRookFigure != nullptr &&
                               kingRookFigure->getType() == LADYA &&
            ...
            Ladya *queenRook = (queenRookFigure != nullptr &&
                                queenRookFigure->getType() == LADYA &&
    ...
    bool move(const Coordinates &coordinates) override {
        if (isValidMove(coordinates)) {
            ...
            if (!hadMoved && curY == coordinates.getY() &&
                (dx == 2 || dx == -2)) {
                int rookX = (dx == 2) ? 7 : 0;
                ...
                Ladya *rook =
                    (rookFigure != nullptr && rookFigure->getType() == LADYA)
                        ? dynamic_cast<Ladya *>(rookFigure)
                        : nullptr;
                if (rook != nullptr) {
                    Coordinates rookTarget(rookTargetX, curY);
                    this->getGameboard()->moveFigure(rook->getCoordinates(),
                                                     rookTarget);
...
class ChessFacade {
  ...
  public:
    ...
    void fillBoard() {
        ...
        for (int i = 0; i < 8; i++) {
            gameboard->setFigure(
                new Pawn(i, WHITE, gameboard, Coordinates(i, 1)));
        }
        ...
        // Ladya
        gameboard->setFigure(
            new Ladya(16, WHITE, gameboard, Coordinates(0, 0)));
        gameboard->setFigure(
            new Ladya(17, WHITE, gameboard, Coordinates(7, 0)));

        gameboard->setFigure(
            new Ladya(18, BLACK, gameboard, Coordinates(0, 7)));
        gameboard->setFigure(
            new Ladya(19, BLACK, gameboard, Coordinates(7, 7)));

        // Elephants
        gameboard->setFigure(
            new Elephant(20, WHITE, gameboard, Coordinates(2, 0)));
        gameboard->setFigure(
            new Elephant(21, WHITE, gameboard, Coordinates(5, 0)));

        gameboard->setFigure(
            new Elephant(22, BLACK, gameboard, Coordinates(2, 7)));
        gameboard->setFigure(
            new Elephant(23, BLACK, gameboard, Coordinates(5, 7)));

        // Queens
        gameboard->setFigure(
            new Queen(24, WHITE, gameboard, Coordinates(3, 0)));
        gameboard->setFigure(
            new Queen(25, BLACK, gameboard, Coordinates(3, 7)));

        // Horses
        gameboard->setFigure(
            new Horse(26, WHITE, gameboard, Coordinates(1, 0)));
        gameboard->setFigure(
            new Horse(27, WHITE, gameboard, Coordinates(6, 0)));

        gameboard->setFigure(
            new Horse(28, BLACK, gameboard, Coordinates(1, 7)));
        gameboard->setFigure(
            new Horse(29, BLACK, gameboard, Coordinates(6, 7)));
